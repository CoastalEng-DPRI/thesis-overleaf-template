...
\ifjapanese
  \documentclass[dvipdfmx,a4j,twoside,openright,11pt]{jreport} % 両面印刷の場合．余白を綴じ側に作って右起こし．
...
% BibTeX style
%\bibliographystyle{jplain} % 著者アルファベット順
\bibliographystyle{junsrt}  % 論文中の引用順

\bindermode  % バインダー用余白設定

% 日本語情報（必要なら）
\jclass  {学位論文}                             % 論文種別
\jtitle    {学位論文用 \LaTeX\ テンプレート}    % タイトル．改行する場合は\\を入れる
\juniv    {〇〇大学}                  % 大学名
\jfaculty  {大学院△△研究科}               % 学部，学科
\jauthor  {ほげ山 ふう助}                       % 著者
\jhyear  {24}                                   % 平成○年度
\jsyear  {2012}                                 % 西暦○年度
\jkeyword  {\LaTeX，テンプレート，学位論文}     % 論文のキーワード
\jproject{インタラクションデザインプロジェクト} %プロジェクト名
\jdate{2013年1月}

% 英語情報（必要なら）
\eclass  {Thesis or Dissertation}            % 論文種別
\etitle    {A \LaTeX Template for Thesis or Dissertation}      % タイトル．改行する場合は\\を入れる
\euniv  {The University of XXX}                             % 大学名
\efaculty  {Graduate School of YYY}  % 学部，学科
\eauthor  {Fusuke Hogeyama}                           % 著者
\eyear  {2012}                                        % 西暦○年度
\ekeyword  {\LaTeX, Templete, Thesis}          % 論文のキーワード
